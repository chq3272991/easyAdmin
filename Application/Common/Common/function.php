...
/**
 * @return bool 判断用户是否移动端
 * 2016.9.10
 */
function is_mobile()
{
	//strtolower将所有字符转换为小写
	$agent = strtolower($_SERVER['HTTP_USER_AGENT']);
	//stripos不区分大小写   strpos区分大小写
...
if(!function_exists('get_page')){
	function get_page(&$m,$where,$pagesize=10){
		...
		$p=new Think\Page($count,$pagesize,'8');   // 第四个参数:分页栏每页显示的页数 如8 , 5
		$p->lastSuffix=false;
 ...
 * @param $m 模型，引用传递
 * @param $where 查询条件
 * @param $p 指定url中page的参数
 * @param int $pagesize 每页查询条数
 * @return \Think\Page
 */
if(!function_exists('get_page_right')) {
	function get_page_right(&$m,$where,$pagesize=10){
		$m1=clone $m;//浅复制一个模型
		$count = $m->where($where)->count();//连惯操作后会对join等操作进行重置
		$m=$m1;//为保持在为定的连惯操作，浅复制一个模型

		$p=new Think\Page($count,$pagesize,'5'); // 第四个参数:分页栏每页显示的页数 如8 , 5
		$p->lastSuffix=false;
		$p->setConfig('header','<li class="rows">&nbsp;&nbsp;第<b>%NOW_PAGE%</b>页/共<b>%TOTAL_PAGE%</b>页</li>');
		$p->setConfig('prev','<<');
		$p->setConfig('next','>>');
 ...
 */
if(!function_exists('is_str_null')){
	function is_str_null($str){
